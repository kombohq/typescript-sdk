 ...
 */
export function hrisGetTimesheets(
  client: KomboCore,
  ...
  PageIterator<
    Result<
      operations.GetHrisTimesheetsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetHrisTimesheetsRequest$outboundSchema.optional().parse(
        value,
      ),
  ...
  const path = pathToFunc("/hris/timesheets")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetHrisTimesheetsResponse$inboundSchema, {
      key: "Result",
    }),
