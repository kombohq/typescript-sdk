...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetHrisPerformanceReviewsRequest$outboundSchema.optional()
        .parse(value),
  ...
  const path = pathToFunc("/hris/performance-reviews")();

  const query = queryJoin(
    encodeFormQuery({
      "ids": payload?.ids,
      "remote_ids": payload?.remote_ids,
      "review_cycle_ids": payload?.review_cycle_ids,
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetHrisPerformanceReviewsResponse$inboundSchema, {
      key: "Result",
    }),
