 ...
 */
export function generalSendPassthroughRequest(
  client: KomboCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.PostPassthroughToolApiRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    tool: encodeSimple("tool", payload.tool, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.PostPassthroughToolApiPositiveResponse$inboundSchema),
