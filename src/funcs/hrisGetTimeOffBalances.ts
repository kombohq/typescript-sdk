 ...
 */
export function hrisGetTimeOffBalances(
  client: KomboCore,
  ...
  PageIterator<
    Result<
      operations.GetHrisTimeOffBalancesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetHrisTimeOffBalancesRequest$outboundSchema.optional().parse(
        value,
      ),
  ...
  const path = pathToFunc("/hris/time-off-balances")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetHrisTimeOffBalancesResponse$inboundSchema, {
      key: "Result",
    }),
