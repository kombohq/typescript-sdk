 ...
 */
export function atsGetAtsAttachments(
  client: KomboCore,
  ...
  PageIterator<
    Result<
      operations.GetAtsAttachmentsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAtsAttachmentsRequest$outboundSchema.optional().parse(
        value,
      ),
  ...
  const path = pathToFunc("/ats/attachments")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAtsAttachmentsResponse$inboundSchema, {
      key: "Result",
    }),
