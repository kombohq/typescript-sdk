 ...
 */
export function atsGetApplications(
  client: KomboCore,
  ...
  PageIterator<
    Result<
      operations.GetAtsApplicationsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAtsApplicationsRequest$outboundSchema.optional().parse(
        value,
      ),
  ...
  const query = queryJoin(
    encodeFormQuery({
      "current_stage_ids": payload?.current_stage_ids,
      "ids": payload?.ids,
      "job_ids": payload?.job_ids,
      "job_remote_ids": payload?.job_remote_ids,
    ...
    encodeFormQuery({
      ...
      "remote_created_after": payload?.remote_created_after,
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAtsApplicationsResponse$inboundSchema, {
      key: "Result",
    }),
