...
import { KomboDefaultError } from "../models/errors/kombodefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | KomboDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | KomboDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new KomboDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new KomboDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
